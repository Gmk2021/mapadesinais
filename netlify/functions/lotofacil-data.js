const HISTORY_URL = "https://raw.githubusercontent.com/eitchtee/loterias.json/main/data/lotofacil.json";
...
function dezenas(values = []) {
  return [...new Set(values.map(Number).filter(n => Number.isInteger(n) && n >= 1 && n <= 25))].sort((a, b) => a - b);
}
...
function parseHistory(payload) {
  ...
  })).filter(x => Number.isInteger(x.concurso) && x.dezenas.length === 15)
...
function parseCaixa(payload) {
  ...
  if (!Number.isInteger(concurso) || nums.length !== 15) return null;
  ...
  try {
    ...
    try {
      ...
      if (latest && !draws.some(d => d.concurso === latest.concurso)) {
        ...
        fonte += " | último conferido na API Caixa";
      }
    } catch (error) {
